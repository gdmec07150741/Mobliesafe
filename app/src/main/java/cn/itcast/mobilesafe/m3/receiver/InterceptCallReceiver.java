...
public class InterceptCallReceiver extends BroadcastReceiver{
                ...
                case TelephonyManager.CALL_STATE_RINGING:
                    ...
                    if(blackContactMode==1||blackContactMode==3){
                        ...
                        context.getContentResolver().registerContentObserver(uri,
                                true,new CallLogObserver(new Handler(),mIncomingNumber,
                                        context));
                        endCall(context);
                    }
                    break;
    ...
    public void deleteCallLog(String incomingNumber,Context conext){
        ContentResolver resolver=conext.getContentResolver();
        Uri uri=Uri.parse("content://call_log/calls");
        Cursor cursor=resolver.query(uri,new String[]{"_id"},
                "number=?",new String[]{incomingNumber},"_id desc limit 1");
