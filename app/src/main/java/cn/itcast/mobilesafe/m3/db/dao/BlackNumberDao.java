package cn.itcast.mobilesafe.m3.db.dao;
...
import cn.itcast.mobilesafe.m3.db.BlackNumberOpenHelper;
...
public class BlackNumberDao {
    ...
    public List<BlackContactInfo>getPageBlackNumber(int pagenumber,int pagesize){
        ...
        List<BlackContactInfo> mBlackContactInfo=new ArrayList<BlackContactInfo>();
        while(cursor.moveToNext()){
            BlackContactInfo info=new BlackContactInfo();
            info.phoneNumber=cursor.getString(0);
            info.mode=cursor.getInt(1);
            info.contactName=cursor.getString(2);
            mBlackContactInfo.add(info);
        }
        cursor.close();
        db.close();
        SystemClock.sleep(30);
        return mBlackContactInfo;
    }
